...
/**
 * @author NuStar
 * @since 2025/8/6 21:52
 */
@Component
@DependsOn(properties = @DependsOn.Property(key = NuStarPartyProperties.IS_SUB_PLUGIN, value = "false"))
@SuppressWarnings("unused")
public class CreatePartyButton extends AbstractPartyButton {
    @Override
    protected MenuExecuteResult doExecute(PartyMenuContext ctx) {
        Player holder = ctx.getHolder();
        if (NuStarPartyAPI.createParty(holder)) {
            PartyMenuAPI.openMyPartyMenu(holder);
            return MenuExecuteResult.success();
        }
        return MenuExecuteResult.failure();
    }
