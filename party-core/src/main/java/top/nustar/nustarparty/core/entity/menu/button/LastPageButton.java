...
/**
 * @author NuStar
 * @since 2025/8/6 22:24
 */
@Component
@DependsOn(properties = @DependsOn.Property(key = NuStarPartyProperties.IS_SUB_PLUGIN, value = "false"))
@SuppressWarnings("unused")
public class LastPageButton extends AbstractPartyButton {

    @Override
    protected MenuExecuteResult doExecute(PartyMenuContext ctx) {
        if (ctx.getGui().getCurrentPage() == 1) {
            ctx.getHolder().sendMessage("§c已经是第一页了");
        } else {
            ctx.getGui().lastPage(ctx.getHolder());
        }
