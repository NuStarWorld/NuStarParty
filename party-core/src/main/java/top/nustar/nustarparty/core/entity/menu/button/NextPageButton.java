...
/**
 * @author NuStar
 * @since 2025/8/6 22:25
 */
@Component
@DependsOn(properties = @DependsOn.Property(key = NuStarPartyProperties.IS_SUB_PLUGIN, value = "false"))
@SuppressWarnings("unused")
public class NextPageButton extends AbstractPartyButton {

    @Override
    protected MenuExecuteResult doExecute(PartyMenuContext ctx) {
        if (!ctx.getGui().hasNextPage()) {
            ctx.getHolder().sendMessage("§a没有下一页了");
        } else {
            ctx.getGui().nextPage(ctx.getHolder());
        }
